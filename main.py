...
# def compute_similarity(e_img_cat, e_text_cat, max_k, threshold, blacklist_img_url, images_df):
#     similarity = get_similarity(e_img_cat, e_text_cat)
#     if max_k is not None:
#         top_k_indices = similarity.topk(max_k).indices.ravel()
#         top_idx = np.unique(top_k_indices)
#     else:
#         top_idx = np.arange(similarity.shape[1])  # All indices

#     similarity_values = similarity[:, top_idx].max(0).values
#     valid_indices = top_idx[similarity_values > threshold]
#     valid_idx = [idx.item() for idx in valid_indices]

#     filtered_items = images_df.iloc[valid_idx]
#     filtered_items = filtered_items[~filtered_items.index.isin(blacklist_img_url)]
#     similar_items = filtered_items.apply(create_item_data, axis=1).tolist()
...
# def handle_list_ids_results(list_ids):
#     results = []
...
def handle_query_results(query, language, results):
...
def index():
    ...
    results = []
    handle_query_results(query, language, results)
